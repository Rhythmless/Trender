	...
	Chart.defaults.global.tooltips = {
		...
		callbacks: {
			...
			label: function(tooltipItem, data) {
				var datasetLabel = data.datasets[tooltipItem.datasetIndex].label || '';
				// Mychaylo
                		//if(parseInt(tooltipItem.yLabel)=='0') return
				// \u200e left-to-right mark ensures that right-to-left
				// text doesnt change the order of our punctuation
				...
				if (this._options.tooltips.mode === 'single') {
					...
					helpers.each(this._data.datasets, function(dataset, datasetIndex) {
						...
						if (currentElement) {
							...
							// ignore zero values
							if (!yScale) return;
							var yLabel = yScale.getLabelForIndex(currentElement._index, currentElement._datasetIndex);
							if (!yLabel) return
					...
					helpers.each(this._active, function(active) {
						if (active) {
							// ignore zero values
							var yScale = active._yScale || active._scale; // handle radar || polarArea charts
							if (!yScale) return;
							var yLabel = yScale.getLabelForIndex(active._index, active._datasetIndex);
							if (!yLabel) return
